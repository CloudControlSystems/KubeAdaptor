...
kubectl create ns montage
while [ true ]
do
  state=`kubectl get ns | grep "montage"|awk '{print $2}'`
  if [ $state == "Active" ];then
    echo "create montage-workflow namespace successful."
    break
  fi
done
...
kubectl apply -f priority-job5678910.yaml
while [ true ]
do
  accbool=1
  i=0
...
kubectl delete -f priority-job5678910.yaml
while [ true ]
do
  state=`kubectl get pods -A |grep "task"| awk '{print $4}'`
  if [ ! $state ];then
    echo "delete priority-job5678910.yaml successful."
    break
  fi
done
...
kubectl delete ns montage
while [ true ]
do
  state=`kubectl get ns | grep "workflow"|awk '{print $2}'`
  if [ ! $state ];then
    echo "delete workflow namespace 'montage' successful."
    break
  fi
done
...
mv usage.txt argo-Montage$j.txt
